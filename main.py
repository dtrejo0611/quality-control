# -*- coding: utf-8 -*-
"""
Created on Mon Mar 24 15:48:35 2025

@author: dtrej
"""

import sys
from interfazB import Ui_MainWindow
...
class aplicacionWindow(QMainWindow):
    def __init__(self):
        ...
        self.dialogo.guardarElementoPresenciaButton.clicked.connect(self.guardarROIPresencia)
        self.dialogo.cargarImagenTest.clicked.connect(self.cargarImagenTestSim)
        self.dialogo.realizarTestButton.clicked.connect(self.realizarTest)
    ...
    def capturarImagen(self):
        ...
        self.img, height, width, bytesPerLine = visionLib.capturarImagenShow(int(camara))
    ...
    def habilitarTest(self):
        if (self.dialogo.activarColorChB.isChecked() or self.dialogo.activarPresenciaChB.isChecked() == True):
    ...
    def cargarImagenTestSim(self):
        rutaT = selectFile()
        self.imgTest, height, width, bytesPerLine = visionLib.capturarImagenSim(rutaT)
    ...
    def takePhoto(self):
        camara = self.dialogo.camarasCBox.currentText()
        self.imgTest, height, width, bytesPerLine = visionLib.capturarImagenShow(int(camara))
        self.mostrarImagenTest(self.imgTest, height, width, bytesPerLine)
        self.imgTestShow = self.imgTest
    ...
    def realizarTest(self):
        ...
        self.dialogo.porcentajeColorResultado.setText(str(round(resultColor[1],2)))
        self.dialogo.resultadoPresenciaLabel.setText(resultPresencia[0])
        self.dialogo.porcentajePresenciaResultado.setText(str(round(resultPresencia[1])))
